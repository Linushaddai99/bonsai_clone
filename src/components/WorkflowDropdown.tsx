...
const WorkflowDropdown: React.FC<WorkflowDropdownProps> = ({details, from}) => {
  return (
    <div>
        <div className={`${from === 'desktop' ? ' w-[650px] top-5 left-[340px] shadow-xs h-[530px]' : 'h-full top-5 left-0 w-full' } bg-white p-6 absolute`}>
            <div className={`${from === 'desktop' ? 'space-y-4 mt-4 grid grid-cols-2' : ''}`}>
                {details.map((detail, index) => (
                    <div key={index} className='flex items-start  w-[300px] mb-10'>
                        ...
                        <div>
                            ...
                            {from === 'desktop' && <p className='text-[#4C4D5F] text-sm'>{detail.description}</p>}
                        </div>
                    </div>
                ))}
            </div>
